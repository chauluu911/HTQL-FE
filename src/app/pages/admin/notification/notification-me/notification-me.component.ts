...
@Component({
  selector: 'app-notification-me',
  templateUrl: './notification-me.component.html',
  styleUrls: ['./notification-me.component.scss'],
})
export class NotificationMeComponent implements OnInit {
  ...
  NOTIFICATION_TYPE_SEEN_STATUS = NOTIFICATION_TYPE_SEEN_STATUS;
  ...
  getLimitLength(value = '', length?: number): string {
    return CommonUtil.getLimitLength(value, length ? length : 25);
  }
      ...
      if (result && result?.success) {
        ...
        const request = {
          ...
          expectedEndDate: result?.value.endDate
            ...
            : null,
          sendStartAt: result?.value.startDate
            ? CommonUtil.getStartOfDay(startDate)
            : null,
          sendEndAt: result?.value.endDate
            ? CommonUtil.getEndOfDay(endDate)
            : null,
  ...
  loadData(request: any): void {
    ...
    this.notificationService
      .searchMe(CommonUtil.formatParams(param), true)
      .subscribe(
        (response) => {
  ...
  onSearchForm(): void {
    ...
    const request = {
      ...
      expectedEndDate: this.formSearchNoti?.value.endDate
        ? CommonUtil.getEndOfDay(endDate)
        : null,
      sendStartAt: this.formSearchNoti?.value.startDate
        ? CommonUtil.getStartOfDay(startDate)
        : null,
      sendEndAt: this.formSearchNoti?.value.endDate
        ? CommonUtil.getEndOfDay(endDate)
        : null,
      isRead: this.formSearchNoti?.value.isRead,
    };
